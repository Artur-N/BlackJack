...
string[] cardRank = new string[] {"туз", "2", "3", "4", "5", "6", "7", "8", "9", "10", "валет", "дама", "король"};
...
string FindWinner(int result1, int result2)
{
    ...
    if (result1 == 21) winner = "Блек-джек. Игрок выиграл!";
    if (result1 > 21 && result2 < 21) winner = "Перебор, выйграло казино.";
    if (result2 == 21) winner = "Игрок проиграл.";
    if (result2 > 21 && result1 < 21) winner = "Победа игрока!";
    if (result1 > result2 && result1 < 21) winner = "Победа Игрока!";
    if (result1 < result2 && result2 < 21) winner = "Игрок проиграл.";
    return winner;
}
